...
def get_tics(pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth) as doc_lym:
         tics_lym={}
         for row_lym in doc_lym:
             tics_lym[row_lym.split('"')[3].lower()]=row_lym.split('"')[1].lower()
         return tics_lym
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    tic1=os.path.join(DATDIR,f"{tic}_prc.dat")
    with open(tic1) as doc_lym:
        rows = []
        for row_lym in doc_lym:
            rows.append(row_lym.split('\n')[0].lower())
    return rows
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for i in tickers_lst:
        ...
        for j in range(len(read_dat(i))):
            dict_lym2=line_to_dict(read_dat(i)[j])
            data.append(dict([(key,dict_lym2[key]) for key in col_lst]))
        dict_lym1[i]={'exchange': tic_exchange_dic[i], 'data':data}
    return dict_lym1
